...
class TweetParser(object):
    ...
    def set_tweet_criteria(self, since_date='2020-06-15', until_date='2020-07-01', handle=None, max_tweets=4):
    ...
    def clean_tweet(self, tweet):
        ...
        """
        return ' '.join(re.sub('(@[A-Za-z0-9]+)|([^0-9A-Za-z \t]) |(\w+:\/\/\S+)| ((www\.[^\s]+)|(https?://[^\s]+)|(.pic.[^\s]+))', " ", tweet.text).split())
